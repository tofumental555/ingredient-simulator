...
const NATURE_SPEED_UP = 1.0;
const NATURE_SPEED_DOWN = 1.0;
const NATURE_FOOD_UP = 1.0;
const NATURE_FOOD_DOWN = 1.0;
const NATURE_SKILL_UP = 1.0;
const NATURE_SKILL_DOWN = 1.0;
...
// 1日エナジー計算
// ================================
function calcDailyEnergy(pokemon, level, subs, pattern) {
  ...
  const t = (h * levelCoef * subTime) / BASE_DIVISOR;
  const hpd = 86400 / t;
...
document.getElementById("calcButton").addEventListener("click", () => {
  ...
  const pokemon = pokemonDB[0]; // ← UI にポケモン選択が無いので固定

  const r = calcDailyEnergy(pokemon, 50, subs, pattern);
  displayResult(r, pokemon.name);
});
